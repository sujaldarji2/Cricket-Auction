...
// Create auction tables if not exist
...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        // Place Bid (Optional - agar future mein chahiye to rakha hai)
        if ($_POST['action'] == 'place_bid') {
            // Tumhara purana place_bid code yahan rakh sakte ho
            // Abhi ke liye comment kiya hai kyunki modal use kar rahe hain
            echo json_encode(['success' => false, 'message' => 'Bidding is disabled in current mode']);
            exit();
        }

        // ==================== MARK SOLD (Clean & Updated) ====================
        if ($_POST['action'] == 'mark_sold') {
            ...
            if ($team_data && $team_data['players_bought'] >= $team_data['max_players']) {
                echo json_encode([
                    'success' => false,
                    'message' => 'Cannot sell: Team has reached maximum player limit!'
                ]);
                exit();
            }

            // Get existing bid history
            $result = $conn->query("SELECT bid_history FROM auction_status WHERE player_id = $player_id");
            $bid_history = ($result && $result->num_rows > 0) ? ($result->fetch_assoc()['bid_history'] ?? '[]') : '[]';
        ...
        if ($_POST['action'] == 'mark_unsold') {
            ...
            // Revert team stats if previously sold
        ...
        if ($_POST['action'] == 'reset_auction') {
            ...
            if ($confirm === 'true') {
                ...
                echo json_encode(['success' => true, 'message' => 'Auction has been reset successfully!']);
            } else {
...
// Fetch Categories, Captain Players & Teams
$categories = $conn->query("SELECT * FROM categories ORDER BY category_name");

$captain_players = $conn->query("SELECT p.*, c.category_name 
                                 FROM players p 
                                 JOIN categories c ON p.category_id = c.id 
                                 ORDER BY p.player_name");

$teams_result = $conn->query("SELECT id, team_name, team_short_code, team_color, logo_url,
...
while ($team = $teams_result->fetch_assoc()) {
    $logo_display = !empty($team['logo_url']) ? 'uploads/team_logos/' . basename($team['logo_url']) : '';
    $teams_array[] = [
        'id' => intval($team['id']),
        'name' => $team['team_name'],
        'logo' => $logo_display,
...
$progress_result = $conn->query("SELECT 
    COUNT(*) as total,
    ...
    <div class="admin-container">
        ...
        <div class="main-content">
            <div class="auction-container">
                ...
                <!-- Right Panel - Auction Controls (Updated) -->
                <!-- Right Panel - Auction Controls -->
<div class="right-panel">
    ...
    <!-- Increment Section (Optional - chahe to hata sakte ho) -->
    <div class="increment-section">
        <div class="section-title">Bid Increment (Optional)</div>
    ...
    <!-- Teams List with Remaining Money -->
    <div class="section-title" style="margin-top: 20px;">Teams & Remaining Purse</div>
    <div class="teams-list" id="teamsList">
        <!-- JavaScript se dynamically fill hoga -->
    </div>
    ...
    <!-- ====================== NEW SOLD CONFIRMATION MODAL ====================== -->
    <div class="modal" id="soldModal" style="display: none;">
        <div class="modal-content" style="max-width: 480px; padding: 30px;">
            <h2><i class="fas fa-gavel" style="color: #27ae60;"></i> Mark Player as SOLD</h2>
            <p id="modalPlayerName" style="font-size: 19px; font-weight: bold; margin: 15px 0 20px 0;"></p>

            <label><strong>Select Team:</strong></label>
            <select id="soldTeamSelect" style="width: 100%; padding: 12px; border-radius: 8px; margin-bottom: 20px;">
            ...
            <label><strong>Final Sold Price (₹):</strong></label>
            <input type="number" id="finalBidPrice" 
                   style="width: 100%; padding: 12px; font-size: 18px; border-radius: 8px; margin-bottom: 20px;"
                   placeholder="Enter final bid amount" min="1000" step="1000">

            <div style="display: flex; gap: 12px;">
                <button onclick="closeSoldModal()" 
                        style="flex: 1; padding: 14px; background: #95a5a6; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
                    Cancel
                </button>
                <button onclick="confirmSold()" 
                        style="flex: 1; padding: 14px; background: #27ae60; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
                    Confirm SOLD
                </button>
            </div>
        </div>
    ...
    <!-- Modal CSS (agar alag style tag mein nahi hai to yahan add kar do) -->
    <!--<style>
        .modal {
            position: fixed;
            top: 0; left: 0;
...
document.addEventListener('DOMContentLoaded', function() {
    loadNextPlayer();
    updateStats();
    renderTeamsWithRemaining();
});

// Clear captains
function clearAllCaptains() {
    document.querySelectorAll('input[name="captains[]"]').forEach(cb => cb.checked = false);
}
...
document.addEventListener('keydown', function(e) {
    ...
    if ((e.key === 's' || e.key === 'S') && currentPlayer) openSoldModal();
    if ((e.key === 'u' || e.key === 'U') && currentPlayer) markUnsold();
});

// ====================== SOLD MODAL ======================
function openSoldModal() {
    if (!currentPlayer) return;
    document.getElementById('modalPlayerName').innerHTML = 
        `${currentPlayer.player_name} <small style="color:#7f8c8d;">(Base: ₹${currentPlayer.base_price.toLocaleString('en-IN')})</small>`;

    const defaultPrice = Math.max(currentPlayer.current_bid || currentPlayer.base_price, currentPlayer.base_price || 10000);
    document.getElementById('finalBidPrice').value = defaultPrice;
...
function confirmSold() {
    const teamSelect = document.getElementById('soldTeamSelect');
    const teamId = parseInt(teamSelect.value);
    const soldPrice = parseFloat(document.getElementById('finalBidPrice').value);
    const selectedOption = teamSelect.options[teamSelect.selectedIndex];
    const teamName = selectedOption.textContent.split('(')[0].trim();

    if (!soldPrice || soldPrice < (currentPlayer.base_price || 1000)) {
        alert("❌ Final sold price must be ≥ Base Price!");
        return;
    }

    if (parseInt(selectedOption.dataset.players) >= parseInt(selectedOption.dataset.max)) {
        alert("❌ This team has reached maximum player limit!");
        return;
    }

    if (!confirm(`Confirm selling ${currentPlayer.player_name} to ${teamName} for ₹${soldPrice.toLocaleString('en-IN')}?`)) return;

    fetch('auction-control.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: `action=mark_sold&player_id=${currentPlayer.id}&sold_price=${soldPrice}&sold_to=${encodeURIComponent(teamName)}&team_id=${teamId}`
    })
    .then(res => res.json())
    .then(data => {
        if (data.success) {
            closeSoldModal();
            alert(`✅ ${currentPlayer.player_name} sold successfully to ${teamName}!`);
            
            updateStats();
            updateTeamRemaining(teamId, soldPrice);
            loadNextPlayer();

            // Player Status Page ko new tab mein khol do (latest data ke saath)
            setTimeout(() => {
                window.open('player-status.php', '_blank');
            }, 700);
        } else {
...
// ====================== RIGHT PANEL TEAMS ======================
function renderTeamsWithRemaining() {
    ...
    teams.forEach(team => {
        ...
        <div class="team-item">
            <div class="team-label">
                ${team.logo ? `<img src="../${team.logo}" alt="${team.name}" style="width:35px;height:35px;border-radius:50%;object-fit:cover;">` : `<i class="fas fa-shield-alt" style="font-size:30px;color:#3498db"></i>`}
                <div class="team-info">
                    <div class="team-name">${team.name}</div>
                    <div class="team-purse" id="purse-${team.id}" 
                         data-remaining="${team.remaining}" 
                         data-players="${team.players_bought}" 
                         data-max="${team.max_players}">
                        ₹${Number(team.remaining).toLocaleString('en-IN')} left 
                        (${team.players_bought}/${team.max_players} players)
                    </div>
                </div>
            </div>
        </div>`;
...
function updateTeamRemaining(teamId, soldPrice) {
    const purseElement = document.getElementById(`purse-${teamId}`);
    if (!purseElement) return;

    let currentRemaining = parseFloat(purseElement.dataset.remaining || 0);
    let newRemaining = Math.max(0, currentRemaining - soldPrice);
    let newPlayers = parseInt(purseElement.dataset.players || 0) + 1;

    purseElement.innerHTML = `
        ₹${newRemaining.toLocaleString('en-IN')} left 
        (${newPlayers}/${purseElement.dataset.max} players)
    `;
    purseElement.dataset.remaining = newRemaining;
    purseElement.dataset.players = newPlayers;
}

// ====================== RANDOM MODE + OTHER FUNCTIONS ======================
function toggleRandomMode() {
    randomMode = document.getElementById('randomMode').checked;
    loadNextPlayer();        // Important: Random mode change pe player reload
}
...
function displayPlayer(player) {
    ...
    document.getElementById('currentBidder').innerHTML = player.current_team ? `<span>${player.current_team}</span>` : '';

    window.bidHistory = player.bid_history || [];
    displayBidHistory();
...
function markUnsold() {
    if (!currentPlayer) return;
    if (!confirm('Mark this player as UNSOLD? This will revert team stats.')) return;
    ...
    .then(data => {
        if (data.success) {
            ...
            // Player Status Page ko new tab mein khol do
            setTimeout(() => {
                window.open('player-status.php', '_blank');
            }, 1000);
        } else {
...
function displayBidHistory() {
    let html = '';
    window.bidHistory.slice().reverse().forEach(bid => {
        const timeStr = new Date(bid.time).toLocaleTimeString();
...
function resetAuction() {
    if (!confirm('Reset entire auction?')) return;
    fetch('auction-control.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
...
function updateStats() {
    ...
    .then(data => {
        if (data.success) {
            ...
            const pending = parseInt(data.stats.pending) || 0;
            const sold    = parseInt(data.stats.sold)    || 0;
            const unsold  = parseInt(data.stats.unsold)  || 0;

            // === Update Numbers ===
            document.getElementById('totalPlayers').textContent   = total;
            document.getElementById('pendingPlayers').textContent = pending;
            document.getElementById('soldPlayers').textContent    = sold;
            document.getElementById('unsoldPlayers').textContent  = unsold;

            // === Update Progress Bars (Safe & Reliable Way) ===
            if (total > 0) {
                const pendingPercent = (pending / total) * 100;
                const soldPercent    = (sold / total) * 100;
                const unsoldPercent  = (unsold / total) * 100;

                // Find all progress-fill elements in order
                const progressBars = document.querySelectorAll('.progress-item .progress-fill');

                if (progressBars.length >= 3) {
                    progressBars[0].style.width = pendingPercent + '%';   // Pending
                    progressBars[1].style.width = soldPercent + '%';      // Sold
                    progressBars[2].style.width = unsoldPercent + '%';    // Unsold
                }
            } else {
                // Agar total 0 hai to sab bars 0 kar do
